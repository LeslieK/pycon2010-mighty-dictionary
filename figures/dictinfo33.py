...
"""
property() returns an array of size dk_size starting at address _dk_entries. 
(POINTER creates a pointer to the entire array; 
the pointer is dereferenced (using [0]) to return the entire array); 
the code then accesses the ith element of the array)
"""
...
def probe_steps(keys, key):
    ...
    for k in list(d):  # this line has complexity n**2; for each k, it iterates through all keys
        del d[k]
...
def probe_all_steps(keys):
    ...
    d = dict.fromkeys(keys)
    o = dictobject(d)
    print('size of hash table: ', o.ma_keys.contents.dk_size)
    m = o.slot_map()
